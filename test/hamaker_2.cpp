...
int main() {
    ...
    for (size_t i = 0; i < 3; i ++) {
        auto y = -2.0 * r_part + double(i) * 2.0 * r_part;
        for (size_t j = 0; j < 3; j ++) {
            auto x = -2.0 * r_part + double(j) * 2.0 * r_part;
            // First cube
            ...
            // Second cube, sightly shifted
            x0.emplace_back(x + r_part, y + r_part, 8.0*r_part);
            x0.emplace_back(x + r_part, y + r_part, 10.0*r_part);
            x0.emplace_back(x + r_part, y + r_part, 12.0*r_part);
